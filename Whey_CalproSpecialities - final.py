...
for key in raw_keys:
    ...
    # Apply color only for critical parameters
    if key in critical_params:
        status_colored = f"{GREEN}{status}{RESET}" if within else f"{RED}{status}{RESET}"
        if not within: critical_compliance = False
    else:
        status_colored = status

    print(f"{key:<25} | {raw_result_str:<15} | {raw_spec_str:<30} | {status_colored}")
    if not within and reason not in ["Spec missing / Cannot compare"] and key not in critical_params:
...
if critical_compliance:
    print(f"{GREEN}{BOLD}This report is ✅ Fully compliant{RESET}")
else:
    print(f"{RED}{BOLD}This report has Non-Compliance{RESET}")
