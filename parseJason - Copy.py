...
param_aliases = {
    ...
    "peroxide": ["peroxide_value", "quality_standard_5_name"],   # 🚫 removed "acid_value"
    "gardner": ["gardner", "colour_gardner"],  # 🚫 removed "color"
...
def get_result_and_spec(key):
    ...
    for k,v in content.items():
        k_lower = k.lower()
        if "method" in k_lower:  # skip method fields
            continue
        if any(alias in k_lower for alias in aliases):
            if "result" in k_lower or "observed" in k_lower or k_lower.endswith("value"):
                found_result = v
            if "spec" in k_lower or "limit" in k_lower or "max" in k_lower or "min" in k_lower:
                found_spec = v
        if k_lower.startswith("test_parameter_") and any(alias in str(v).lower() for alias in aliases):
            suffix = k_lower.replace("test_parameter_","")
            found_result = content.get(f"observed_results_{suffix}", found_result)
            found_spec   = content.get(f"specifications_{suffix}", found_spec)
